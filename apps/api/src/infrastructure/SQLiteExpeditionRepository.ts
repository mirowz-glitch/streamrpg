...
export class SQLiteExpeditionRepository implements ExpeditionRepository {
  ...
  // Sprint Expedition Choice Phase III — mesmo padrão idempotente de
  // ChronicleRepository.insertOnce: o `WHERE approach IS NULL` dentro
  // do próprio UPDATE fecha a janela de corrida sem precisar de uma
  // segunda checagem. `character_id = ?` garante que só o dono da
  // expedição grava; `status != 'completed'` evita gravar numa
  // expedição que já terminou (a identidade não deveria mais mudar
  // nada nesse ponto).
  async setApproach(expeditionId: string, characterId: string, approach: ExpeditionApproach): Promise<boolean> {
    const result = getDb()
      .prepare(
        `UPDATE expeditions SET approach = ?
         WHERE id = ? AND character_id = ? AND approach IS NULL AND status != 'completed'`,
      )
      .run(approach, expeditionId, characterId);
